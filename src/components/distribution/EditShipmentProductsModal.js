...
function EditShipmentProductsModal({ isOpen, toggle, shipment, onSaved }) {
  ...
  const entreCuerosDest = isEntreCuerosLocation(locationId || shipment?.locationId);
  ...
  const handleSubmit = async () => {
    ...
    const payloadLines = lines
      .map((row) => ({
        ...
        hardwareCondition: entreCuerosDest
          ? resolvePayloadHardware(locationId || shipment?.locationId, productsById.get(Number(row.productId)), row.hardwareCondition) || null
    ...
    for (const row of payloadLines) {
      ...
      if (source && lineNeedsDimension(source) && !row.hardwareCondition) {
        const product = getLineProduct(source);
        const kind = stockDimensionKind(locationId || shipment?.locationId, product);
                    ...
                    {needsDimension ? (
                      entreCuerosDest ? (
                        <KioskInventoryDimensionSelect
                          locationId={locationId || shipment?.locationId}
                          product={product}
