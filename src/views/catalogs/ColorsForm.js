...
import { getColorById, createColor, createColors, updateColor } from "services/colorService";

function ColorsForm({ colorId, isOpen, toggle, onSuccess }) {
    ...
    try {
      ...
      if (colorId) {
        ...
        if (colorNames.length === 1) {
          await createColor({ name: colorNames[0] });
        } else {
          await createColors(colorNames);
        }
      }
      onSuccess();
      toggle();
