...
import DieCutListPanel from "./organizer/DieCutListPanel";
  ...
  return (
    <div className="content">
      ...
      <Nav tabs className="mb-3">
        ...
        {/* Va aqui, entre el borrador y el tablero, porque es el paso del flujo: una
            tarea recien creada no tiene corte y no deberia bajar a mesa hasta tenerlo. */}
        <NavItem>
          <NavLink
            role="button"
            className={classnames({ active: activeTab === "diecut" })}
            onClick={() => { setActiveTab("diecut"); org.loadDieCutPending(); }}
          >
            2 · Por troquelar
            {org.dieCutPending.length > 0 && (
              <Badge color="warning" className="ml-1">{org.dieCutPending.length}</Badge>
            )}
          </NavLink>
        </NavItem>
        <NavItem>
      ...
      <TabContent activeTab={activeTab}>
        <TabPane tabId="organize">
          <Row>
            ...
            <Col lg="5" xl="4">
              {/* Crear tarea va primero porque es el primer paso del flujo: se arma la tarea,
                  se marca su troquelado y solo entonces el reparto le da mesa. Con el orden
                  al reves, la accion de arriba era la ultima que tocaba hacer. */}
              <DraftTaskPanel
                ...
                onCreate={async () => {
                  ...
                  // Al tablero ya no: una tarea recien creada no tiene corte, y el paso
                  // siguiente del flujo es decir que productos ya se troquelaron.
                  if (created) {
                    await org.loadDieCutPending();
                    setActiveTab("diecut");
                  }
                }}
        ...
        <TabPane tabId="diecut">
          <DieCutListPanel
            tareas={org.dieCutPending}
            loading={org.loadingDieCut}
            onReload={org.loadDieCutPending}
          />
        </TabPane>
