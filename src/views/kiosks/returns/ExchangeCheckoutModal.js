...
function ExchangeCheckoutModal({
  ...
  const expectedCardAmount = useMemo(() => {
    if (paymentMethod === "TARJETA") return totalDue;
    if (paymentMethod === "MIXTO") return Number(cardAmount || 0);
    return 0;
  }, [paymentMethod, totalDue, cardAmount]);

  useEffect(() => {
    if (paymentMethod === "TARJETA" && totalDue > 0) {
      setCardVoucherAmount(String(totalDue.toFixed(2)));
    }
  ...
  const voucherDiff = requiresCardData
    ? Number(cardVoucherAmount || 0) - expectedCardAmount
    : 0;
  const showVoucherDiff = requiresCardData && Math.abs(voucherDiff) > 0.009;
  ...
  const handleConfirm = () => {
    ...
    onConfirm({
      ...
      phone: String(invoicePhone || "").trim() || null,
      requestInvoice: true,
      reason: reason || null,
    ...
    <Modal isOpen={isOpen} toggle={onClose} centered className="kiosk-pos-checkout-modal">
      ...
      <div className="kiosk-pos-checkout-footer">
        <button type="button" className="kiosk-pos-btn-confirm" onClick={handleConfirm} disabled={!canConfirm}>
          {saving ? "Procesando..." : `Confirmar y facturar ${formatCurrency(totalDue)}`}
        </button>
        <button type="button" className="kiosk-pos-btn-cancel" onClick={onClose} disabled={saving}>
          Cancelar
        </button>
        <p className="kiosk-pos-confirm-hint mb-0">
          Al confirmar se registra el cobro y se certifica la factura electrónica.
        </p>
        {cashInsufficient && (
